...
{#if $openModal}
  <div
    class="fixed top-0 left-0 w-screen h-screen border-b bg-white z-50 overflow-hidden flex flex-col gap-8 p-5 px-8 md:hidden"
  >
    <div class="flex items-center justify-between gap-4 border-b pb-2">
      <h1 class="font-semibold">
        Swiss <span class="text-indigo-400">Molly</span>
      </h1>
      <button
...
{#if y > outerHeight}
  <div
    class="backdrop-blur bg-white/30 fixed top-0 left-0 w-full flex flex-col z-20 px-4 fadeIn"
  >
    <Header />
  </div>
{/if}
